...
4. Route check. Open `tests/Architecture/OneRoutePerFactTest.php` (once step-32 exists). For each fact the row touches, note its interface, its composite (`Composite<Interface>`), and its composition roots. A consumer is typed on the interface and never names an implementation; a new implementation implements the interface, lives in the family's namespace, and is named only at the root. Where the row deletes a route, the ledger's pending entry for it goes too. If the change would name an implementation anywhere but the root, or would bind a consumer to a concrete class, the design has moved the problem: stop and report.

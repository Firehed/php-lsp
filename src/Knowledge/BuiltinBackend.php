...
/**
 * The lowest-precedence {@see SymbolSourceInterface}: the symbols built into
 * PHP and its loaded extensions, described through reflection. It is consulted
 * only after the open-document, workspace, and vendor backends, so a name any
 * of them can resolve never reaches reflection (RFC 1 §5.3).
 *
 * Built-ins are fixed for a given target environment, so a resolved symbol is
 * cached (RFC 1 §5.3). This backend is reflection-backed and therefore
 * describes the *server's* runtime, not the project's target — a known §4.7
 * gap; the interim treats every reflected built-in as available.
 *
 ...
 * reflection-to-info build is this backend, so a separate class only duplicated
 * the source-picking that {@see CompositeSymbolSource} already owns.
 */

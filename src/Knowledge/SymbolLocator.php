 ...
 * `SymbolLocator`).
 *
 * The kind is a parameter rather than a method-per-kind because the two cross-
 * products §5.6 keeps closed are *consumers × kinds* and *backends × kinds*: a new
 * caller resolves through the same entry, whichever namespace it is asking about.
 * It is not redundant with the name, because {@see QualifiedName} is deliberately
 * kind-neutral — `Foo\bar` names a different symbol as a function than as a
 * constant, and only the syntactic position the caller read it from can say which.
 *
